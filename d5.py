...
def construir_serie_mensual(df_std: pd.DataFrame) -> pd.DataFrame:
    ...
    Se deriva el año del "último periodo" como el segundo año de 'anios_comparados' (ej.: 2022-2023).
    """
    ...
    long["fecha"] = pd.to_datetime(
        dict(year=long["anio"].astype(int), month=long["mes"].astype(int), day=1)
    )
...
if serie_m.empty:
    st.info(
        "No se pudo construir la serie mensual. Verifica que el CSV tenga 'Años comparados' y 'Período' con meses."
    )
else:
    # Total mensual de delitos de alto impacto (suma de todos los delitos del archivo)
    total_m = (
    ...
    # Datos reales disponibles hasta 2023 (por el archivo actual)
    real = total_m[(total_m["fecha"] >= "2023-01-01") & (total_m["fecha"] <= "2023-12-01")].copy()

    # Pronóstico: 2024-01 a 2026-01
    ...
    if not fc.empty:
        ...
        fig_band = px.area(
            ...
            x="fecha",
            y="superior",
            labels={"fecha": "Fecha", "superior": "Banda"},
        )
        fig_band2 = px.area(banda, x="fecha", y="inferior")
        # Insertar áreas como trazas (sin estilos explícitos)
        for tr in fig_band.data:
            fig_ts.add_trace(tr)
        for tr in fig_band2.data:
            fig_ts.add_trace(tr)
